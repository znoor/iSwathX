...
#' @return a boxplot of ion intensity correlation between the two libraries and training data set
#' @examples
...
computeIntensityCor <- function(dat1,dat2, method = "spearman", nomod=FALSE)
  ...
  if(nrow(mm) > 5){
    ...
    if(length(list.ms2) > 1){
      ...
      allCordata <- data.frame(allCor)
       ...
       ggsave("Ion_Intensity_Correlation.png", width = 6, height = 4)
       # dev.off()
    }
  }
