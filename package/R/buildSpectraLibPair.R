                           ...
                           consolidateAccession=FALSE, ...)
{
  ...
  {
    warning("Library merging not executed because common peptides are less than 20. Only the seed library is returned.")
	...
	res <- datBaseLib
	  ...
	  if(merge) {
		...
		reliabilityCheckLibrary(datBaseLib, datlib.fin)
		datlib.fin = libraryFormat(datlib.fin)
		datlib.fin = outputLib(datlib.fin, outputFile, outputFormat)
