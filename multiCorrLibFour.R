...
multiCorrLibFour <- function(baseLib,extLib1, extLib2, extLib3,
                              method = "time",
                              cutoff.size = 50,
                              cutoff.r2 = 0.8,
...

{
  ...
  rtcor3 <- ggplot(data = datRTrain3, aes(x = datRTrain3[,2], y = datRTrain3[,3])) +
    geom_point(shape=21, fill="black", color = "gray", size = 2, alpha = 0.5) + 
    
    annotate(geom="text", x=min(datRTrain3[,2])+4, y=max(datRTrain3[,3])-4, label=deparse(r2label), parse = T,
             color="red")+
    geom_smooth(method="lm", color="#ff0000", se = F, fullrange = T) +
    
    scale_x_continuous(name=label1, limits=c(min(datRTrain3[,2]), max(datRTrain3[,2])))+
    scale_y_continuous(name=label4, limits=c(min(datRTrain3[,3]), max(datRTrain3[,3])) )+
    scale_color_brewer(palette="Accent")+
    ggtitle("Retention time correlation between base and external library")+
    theme_classic() +
    theme(plot.title = element_text(size = 12, face = "bold"),
  ...
  corr_stats <- data.frame("Libraries" = c(label2, label3, label4),
  ...
  # corr_stats$index <- c(1,2, 3)
  rtcor <- ggarrange(rtcor1, rtcor2, rtcor3, nrow = 1, ncol = 3)
