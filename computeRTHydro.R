...
#' list.hydrocor <- computeRTHydro(datBaseLib, datExtLib)
############################################################################
#
#      Retention time hydro correlation function (separate from SwathXtend computeRIICor)
...

{
  ...
  if(nrow(datRTrain) > 10){
    ...
    rtcorhydro <- ggplot(data = datRTrain, aes(x = datRTrain[,2], y = datRTrain[,3])) +
      ...
      scale_y_continuous(name= paste0(label1,"RT"), limits=c(min(datRTrain[,3]), max(datRTrain[,3])) )+
    ...
    ggsave("RT_Correlation_Hydro.png", width = 6, height = 5, path = filepath2)
    # dev.off()
    ...
    rtcorhydrolen <- ggplot(data = datRTrain, aes(x = datRTrain[,2], y = datRTrain[,3])) +
      ...
      scale_x_continuous(name=paste0(label1, "Hydrophibicity index"), limits=c(min(datRTrain[,2]), max(datRTrain[,2])))+
      scale_y_continuous(name=paste0(label1,"RT"), limits=c(min(datRTrain[,3]), max(datRTrain[,3])) )+
      scale_color_brewer(palette="Accent")+
      ggtitle("Correlation between Retention time and Hydrophobicity index")+
      theme_classic() +
    ...
    rtcorrtlen <- ggplot(data = datRTrain, aes(x = datRTrain[,4], y= datRTrain[,2])) +
      ...
      annotate(geom="text", x=min(datRTrain[,4])+4, y=max(datRTrain[,2])-4, label=deparse(r2label), parse = T,
               color="red")+
      geom_smooth(method="lm", color="#ff0000", se = F, fullrange = T) +
      scale_x_continuous(name=paste0(label1,"Hydrophibicity & Length"), limits=c(min(datRTrain[,4]), max(datRTrain[,4])))+
      scale_y_continuous(name=paste0(label1,"RT"), limits=c(min(datRTrain[,2]), max(datRTrain[,2])) )+
      
      scale_color_brewer(palette="Accent")+
      ggtitle("Correlation between Retention time and Length")+
      theme_classic() +
      theme(plot.title = element_text(size = 12, face = "bold"),
            text = element_text(size = 12),
            axis.title = element_text(size = 11, face = "bold"),
    ...
    filepath <- getwd()
    filepath2 <- paste0(filepath,"/graphs")
    ggsave("RT_Correlation_HydroNLength.png", width = 6, height = 5, path = filepath2)
    ...
    rmselabel <- bquote(italic(RMSError) ==. (format(rmse.h, digits=2)))
    ...
    filepath <- getwd()
    filepath2 <- paste0(filepath,"/graphs")
    ggsave("RT_residual_Hydro.png", width = 6, height = 5, path = filepath2)
    ...
    rmselabel <- bquote(italic(RMSError) ==. (format(rmse.hl, digits=2)))
    ...
    filepath <- getwd()
    filepath2 <- paste0(filepath,"/graphs")
    ggsave("RT_residual_HydroNLength.png", width = 6, height = 5, path = filepath2)
    ...
    list(datRTrain, rtcorhydro, reshydro)
